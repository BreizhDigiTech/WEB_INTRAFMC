...
export class UnifiedGraphQLService extends GraphQLService {
    ...
    async getOrdersStatistics(startDate: Date, endDate: Date): Promise<OrderStatistics> {
        const query = `
        ...
        `
        const response = await this.request(query, { startDate, endDate })
        return response.orderStatistics
    }
    
    async getRevenueTimelineData(startDate: Date, endDate: Date, groupBy: TimeGrouping = TimeGrouping.MONTH): Promise<RevenueTimeline> {
        const query = `
        ...
        `
        const response = await this.request(query, { startDate, endDate, groupBy })
        return response.revenueTimeline
    }
    
    async getMonthlyRevenueData(months: number = 12): Promise<MonthlyRevenueResponse> {
        const query = `
        ...
        `
        const response = await this.request(query, { months })
        return response.monthlyRevenue
    }
    
    async getDashboardStatistics(period: TimeGrouping = TimeGrouping.MONTH): Promise<JSON> {
        const query = `
            query DashboardStats($period: TimeGrouping) {
                dashboardStats(period: $period)
            }
        `
        const response = await this.request(query, { period })
    ...
    async getOrdersSummaryData(): Promise<OrdersSummary> {
        const query = `
            query OrdersSummary {
                ordersSummary {
                    totalOrders
    ...
    async getUsersSummaryData(): Promise<UsersSummary> {
        const query = `
            query UsersSummary {
                usersSummary {
                    totalUsers
    ...
    async getEcommerceSummaryData(): Promise<EcommerceSummary> {
        const query = `
