<template>
  <div class="overflow-x-auto">
    <table class="table table-zebra w-full">
      ...
      <tbody>
        <tr v-for="product in products" :key="product.id" class="hover:bg-gray-800/30 border-gray-700/50">
          <!-- Checkbox -->
          <td>
            <label>
              <input type="checkbox" class="checkbox checkbox-sm" />
          ...
          <td>
            <div class="flex items-center gap-2">
              <div class="font-semibold" :class="getStockColorClass(product)">
                {{ formatNumber(product.stock) }}
              </div>
              <button @click="openStockModal(product)" class="btn btn-xs btn-ghost text-gray-400 hover:text-white">
            ...
            <div class="w-full bg-gray-700 rounded-full h-1 mt-1">
              <div class="h-1 rounded-full transition-all duration-300" :class="getStockBarClass(product)"
          ...
          <!-- Statut -->
          <td>
            <div class="badge border px-2 py-1" :class="getStatusBadgeClass(getProductStatus(product))">
          ...
          <td>
            <div class="flex items-center gap-1">
              <button @click="$emit('edit', product)" class="btn btn-xs btn-ghost text-blue-400 hover:bg-blue-600/20">
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                </svg>
    ...
    <dialog v-if="showStockModal && selectedProduct" class="modal modal-open">
      <div class="modal-box bg-gray-800 border border-gray-700">
        ...
        <div class="form-control mb-4">
          ...
          <input v-model.number="newStock" type="number"
            class="input input-bordered bg-gray-700 border-gray-600 text-white"
            :placeholder="selectedProduct.stock.toString()" min="0">
        </div>
