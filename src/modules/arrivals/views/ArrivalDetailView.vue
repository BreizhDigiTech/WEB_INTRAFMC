<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
      ...
      <div class="container mx-auto px-4 py-4">
        ...
        <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4">
          <div class="flex items-center space-x-4">
            <button @click="goBack" class="p-2 hover:bg-gray-800 rounded-xl transition-colors">
          ...
          <div class="flex flex-wrap gap-3">
            <button @click="refreshArrival" :disabled="loading"
            ...
            <template v-if="arrival && arrival.status === 'pending'">
              <button @click="validateArrival" :disabled="actionLoading"
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div v-else-if="!loading && !arrival && error" class="flex items-center justify-center py-20">
        <div class="text-center max-w-md">
          <div class="p-4 bg-red-500/20 rounded-2xl mb-4 inline-block">
          ...
          <div class="flex gap-3 justify-center">
            <button @click="loadArrival" class="btn bg-blue-600 hover:bg-blue-700 text-white border-none">
            ...
            <button @click="goBack" class="btn bg-gray-600 hover:bg-gray-700 text-white border-none">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
              </svg>
              Retour à la liste
      ...
      <div v-else-if="!loading && !arrival && !error" class="flex items-center justify-center py-20">
        <div class="text-center max-w-md">
          ...
          <h3 class="text-xl font-semibold text-white mb-2">Arrivage introuvable</h3>
          <p class="text-gray-400 mb-6">
            L'arrivage avec l'ID "{{ arrivalId }}" n'a pas été trouvé.
          </p>
          <button @click="goBack" class="btn bg-blue-600 hover:bg-blue-700 text-white border-none">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
            Retour à la liste
        ...
        <div class="bg-gray-800/50 backdrop-blur-sm rounded-2xl border border-gray-700 overflow-hidden">
          ...
          <div class="overflow-x-auto">
            <table class="table table-zebra w-full">
              ...
              <tbody>
                <tr v-for="arrivalProduct in arrival?.products || []" :key="arrivalProduct.id"
                  ...
                  <td class="text-green-400 font-semibold">
                    {{ arrivalProduct.unit_price ? formatCurrency(arrivalProduct.quantity * arrivalProduct.unit_price) :
                      (arrivalProduct.product?.price ? formatCurrency(arrivalProduct.quantity *
                        arrivalProduct.product.price)
                        : '-') }}
                  </td>
                </tr>
              </tbody>
...
const loadArrival = async () => {
  try {
    ...
    loading.value = true
    error.value = null
    console.log('Loading arrival with ID:', arrivalId.value)
    arrival.value = await arrivalService.getArrival({ id: arrivalId.value })
    console.log('Arrival loaded successfully:', arrival.value)
  } catch (err: any) {
    const errorMessage = err.message || 'Erreur lors du chargement de l\'arrivage'
...
const validateArrival = async () => {
  ...
  try {
    ...
    const updatedArrival = await arrivalService.validateArrival({ id: arrival.value.id })
