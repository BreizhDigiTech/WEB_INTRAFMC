// Utilitaires de formatage pour le module arrivals (basé sur l'API backend)
...
/**
 * Retourne le label d'affichage pour un statut d'arrivage
 */
...
/**
 * Retourne la couleur CSS pour un statut d'arrivage
 */
...
/**
 * Retourne les classes CSS pour le badge de statut d'arrivage
 */
...
/**
 * Calcule le montant total d'un arrivage basé sur ses produits
 */
export function calculateTotalAmount(products: any[]): number {
...
/**
 * Génère une référence d'arrivage automatique
 */
export function generateArrivalReference(): string {
    const date = new Date()
    ...
    return `ARR${year}${month}${day}-${time}`
}
