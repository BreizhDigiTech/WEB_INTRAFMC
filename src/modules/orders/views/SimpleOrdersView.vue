  ...
  <div class="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
      ...
      <div class="container mx-auto px-4 py-4">
        ...
        <div class="flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4">
          ...
          <div class="flex flex-wrap gap-3">
            <!-- Recherche rapide -->
            <div class="relative">
              <input v-model="searchQuery" type="text" placeholder="Rechercher commande, client..."
            ...
            <!-- Actualiser -->
            <button @click="refreshOrders" :disabled="orderStore.loading"
              ...
              <span class="hidden sm:inline ml-2">
                {{ orderStore.loading ? (Object.keys(appliedFilters).length > 0 ? 'Filtrage...' : 'Chargement...') :
                  'Actualiser' }}
              </span>
            </button>
          </div>
    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <!-- Annulées -->
        <div
          class="bg-gradient-to-br from-red-500/10 to-rose-500/10 backdrop-blur-sm border border-red-500/20 rounded-2xl p-6 hover:border-red-500/40 transition-all duration-300">
          <div class="flex items-center justify-between mb-4">
        ...
        <!-- Total avec CA -->
        <div
          class="bg-gradient-to-br from-blue-500/10 to-purple-500/10 backdrop-blur-sm border border-blue-500/20 rounded-2xl p-6 hover:border-blue-500/40 transition-all duration-300">
          <div class="flex items-center justify-between mb-4">
      ...
      <!-- Pagination -->
      <UIPagination v-if="orderStore.totalOrders > 0" :current-page="orderStore.currentPage"
        :total-pages="Math.max(1, orderStore.totalPages)" :total="orderStore.totalOrders" :per-page="orderStore.perPage"
        :from="orderStore.totalOrders > 0 ? ((orderStore.currentPage - 1) * orderStore.perPage) + 1 : 0"
        :to="Math.min(orderStore.currentPage * orderStore.perPage, orderStore.totalOrders)"
        :filtered="Object.keys(appliedFilters).length > 0 || (searchQuery.trim() !== '')" label-plural="commandes"
        :per-page-options="[10, 20, 50]" @go-to-page="goToPage" @next="goToPage(orderStore.currentPage + 1)"
        @prev="goToPage(orderStore.currentPage - 1)" @per-page-change="changePerPage" />
...
// Pagination numérique gérée par le composant UIPagination
